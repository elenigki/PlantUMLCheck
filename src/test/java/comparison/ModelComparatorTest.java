...
public class ModelComparatorTest {
	...
	@Test
	void dependencyInUml_butCodeHasStronger_relaxed_isSuggestion() {
		IntermediateModel code = codeModel();
		...
		Difference d = diffs.get(0);
		assertEquals(IssueKind.RELATIONSHIP_MISMATCH, d.getKind());
		assertEquals(IssueLevel.SUGGESTION, d.getLevel()); // RELAXED: upgrade suggestion
	...
	// --- PARAMETER MATCHING (STRICT expectation) ---
	...
	@Test
	void dependencyOnlyInUml_isError_now() {
		IntermediateModel code = codeModel();
		ClassInfo a = addClass(code, "A", ClassType.CLASS);
		ClassInfo b = addClass(code, "B", ClassType.CLASS);
		...
		Difference d = diffs.get(0);
		assertEquals(IssueKind.RELATIONSHIP_MISMATCH, d.getKind());
		assertEquals(IssueLevel.ERROR, d.getLevel()); // UML-only in family => ERROR
		assertEquals("A -> B", d.getWhere());
		assertEquals("DEPENDENCY", d.getUml());
	...
	@Test
	void generalizationOnlyInUml_relaxed_isError() {
		IntermediateModel code = codeModel();
		...
		assertEquals(IssueLevel.ERROR, d.getLevel()); // now ERROR
	...
	@Test
	void realizationOnlyInUml_relaxed_isError() {
		IntermediateModel code = codeModel();
	...
	// --- MINIMAL (formerly RELAXED_PLUS): member omissions OK; written details
	// must match ---

	@Test
	void attributeTypeWrittenDifferent_minimal_isError() {
		// code & uml models
		...
		ModelComparator cmp = new ModelComparator(CheckMode.MINIMAL);
		List<Difference> diffs = cmp.compare(code, uml);
	...
	@Test
	void methodParamsWritten_minimal_intVsInteger_isErrorAndSuggestion() {
		IntermediateModel code = new IntermediateModel(ModelSource.SOURCE_CODE);
		IntermediateModel uml = new IntermediateModel(ModelSource.PLANTUML_SCRIPT);
		...
		// 2) code overload not drawn in UML (SUGGESTION, public API)
		...
		for (Difference d : diffs) {
			if (d.getKind() == IssueKind.METHOD_MISSING_IN_CODE) {
				...
				assertEquals(IssueLevel.SUGGESTION, d.getLevel(), "code-only method should be SUGGESTION in MINIMAL");
				assertTrue(d.getWhere().startsWith("UserService#find("),
						"where should reference the find(..) signature");
	...
	@Test
	void publicMethodMissingInUml_minimal_isSuggestion() {
		IntermediateModel code = new IntermediateModel(ModelSource.SOURCE_CODE);
		IntermediateModel uml = new IntermediateModel(ModelSource.PLANTUML_SCRIPT);
	...
	@Test
	void relaxed_dependencyInUml_butCodeHasAssociation_suggestion() {
		IntermediateModel code = codeModel();
		ClassInfo a = addClass(code, "A", ClassType.CLASS);
		ClassInfo b = addClass(code, "B", ClassType.CLASS);
		addRel(code, a, b, RelationshipType.ASSOCIATION); // stronger in code
		...
		Difference d = diffs.get(0);
		assertEquals(IssueKind.RELATIONSHIP_MISMATCH, d.getKind());
		assertEquals(IssueLevel.SUGGESTION, d.getLevel()); // now suggestion
		assertEquals("A -> B", d.getWhere());
		assertEquals("DEPENDENCY", d.getUml());
		assertEquals("ASSOCIATION", d.getCode());
	}
	...
	@Test
	void relaxed_realization_vs_generalization_warningPlusError_now() {
		IntermediateModel code = codeModel();
		ClassInfo impl = addClass(code, "Impl", ClassType.CLASS);
		ClassInfo api = addClass(code, "API", ClassType.INTERFACE);
		addRel(code, impl, api, RelationshipType.REALIZATION); // class implements interface
		...
		assertEquals(2, diffs.size(), "Expect missing REALIZATION (WARNING) + extra GENERALIZATION (ERROR)");
		...
		assertTrue(
				diffs.stream()
						.anyMatch(d -> d.getKind() == IssueKind.RELATIONSHIP_MISSING_IN_CODE
								&& "Impl -> API".equals(d.getWhere()) && "GENERALIZATION".equals(d.getUml())
								&& d.getLevel() == IssueLevel.ERROR),
				"GENERALIZATION present only in UML should be ERROR");
	}

	// --- Extra weaker edge in UML (in addition to a matching stronger edge) -> no
	// diffs ---
	@Test
	...
	@Test
	void visibility_attribute_umlLessPublic_strict_isError() {
		IntermediateModel code = codeModel();
		ClassInfo c = addClass(code, "V1", ClassType.CLASS);
		addAttr(c, "id", "int", "+"); // code public

		IntermediateModel uml = umlModel();
		ClassInfo u = addClass(uml, "V1", ClassType.CLASS);
		addAttr(u, "id", "int", "-"); // UML private

		List<Difference> diffs = new ModelComparator(CheckMode.STRICT).compare(code, uml);

		assertEquals(1, diffs.size());
		Difference d = diffs.get(0);
		assertEquals(IssueKind.ATTRIBUTE_MISMATCH, d.getKind());
		assertEquals(IssueLevel.ERROR, d.getLevel());
		assertEquals("V1.attr:id", d.getWhere());
	}

	@Test
	void visibility_method_umlMorePublic_relaxed_isWarning() {
		IntermediateModel code = codeModel();
		ClassInfo c = addClass(code, "V2", ClassType.CLASS);
		addMethod(c, "m", "void", "-", new String[] {}); // code private

		IntermediateModel uml = umlModel();
		ClassInfo u = addClass(uml, "V2", ClassType.CLASS);
		addMethod(u, "m", "void", "+", new String[] {}); // UML public
		...
		assertTrue(d.getWhere().startsWith("V2#m("));
	}

	@Test
	void visibility_method_omitted_strict_isError() {
		IntermediateModel code = codeModel();
		ClassInfo c = addClass(code, "V3", ClassType.CLASS);
		addMethod(c, "size", "int", "+"); // code public

		IntermediateModel uml = umlModel();
		ClassInfo u = addClass(uml, "V3", ClassType.CLASS);
		addMethod(u, "size", "int", ""); // UML visibility omitted

		List<Difference> diffs = new ModelComparator(CheckMode.STRICT).compare(code, uml);

		assertEquals(1, diffs.size());
		assertEquals(IssueLevel.ERROR, diffs.get(0).getLevel());
	}

	@Test
	void visibility_method_omitted_minimal_isSuggestion() {
		IntermediateModel code = new IntermediateModel(ModelSource.SOURCE_CODE);
		IntermediateModel uml = new IntermediateModel(ModelSource.PLANTUML_SCRIPT);

		ClassInfo c = addClass(code, "V4", ClassType.CLASS);
		addMethod(c, "size", "int", "+"); // code public

		ClassInfo u = addClass(uml, "V4", ClassType.CLASS);
		addMethod(u, "size", "int", ""); // UML visibility omitted

		List<Difference> diffs = new ModelComparator(CheckMode.MINIMAL).compare(code, uml);

		assertEquals(1, diffs.size());
		assertEquals(IssueKind.METHOD_MISMATCH, diffs.get(0).getKind());
		assertEquals(IssueLevel.SUGGESTION, diffs.get(0).getLevel());
	}

	@Test
	void relaxed_codeOnlyPublicMethod_isError() {
		IntermediateModel code = codeModel();
		ClassInfo c = addClass(code, "P1", ClassType.CLASS);
		addMethod(c, "export", "String", "+"); // present only in code

		IntermediateModel uml = umlModel();
		addClass(uml, "P1", ClassType.CLASS);

		List<Difference> diffs = new ModelComparator(CheckMode.RELAXED).compare(code, uml);

		assertEquals(1, diffs.size());
		assertEquals(IssueKind.METHOD_MISSING_IN_UML, diffs.get(0).getKind());
		assertEquals(IssueLevel.ERROR, diffs.get(0).getLevel()); // RELAXED doesn't relax members
	}

	@Test
	void ownership_onlyInUml_relaxed_isError() {
		IntermediateModel code = codeModel();
		ClassInfo a = addClass(code, "O1", ClassType.CLASS);
		ClassInfo b = addClass(code, "O2", ClassType.CLASS);
		// no ownership in code

		IntermediateModel uml = umlModel();
		ClassInfo ua = addClass(uml, "O1", ClassType.CLASS);
		ClassInfo ub = addClass(uml, "O2", ClassType.CLASS);
		addRel(uml, ua, ub, RelationshipType.ASSOCIATION);
		...
		assertEquals(IssueKind.RELATIONSHIP_MISMATCH, d.getKind());
		assertEquals(IssueLevel.ERROR, d.getLevel()); // UML-only in family => ERROR
		assertEquals("O1 -> O2", d.getWhere());
	}

	@Test
	void ownership_codeOnly_strict_isError() {
		IntermediateModel code = codeModel();
		ClassInfo a = addClass(code, "O3", ClassType.CLASS);
		ClassInfo b = addClass(code, "O4", ClassType.CLASS);
		addRel(code, a, b, RelationshipType.ASSOCIATION);

		IntermediateModel uml = umlModel();
		addClass(uml, "O3", ClassType.CLASS);
		addClass(uml, "O4", ClassType.CLASS);
		// no edge in UML
	...
	@Test
	void ownership_codeOnly_relaxed_isWarning() {
		IntermediateModel code = codeModel();
		ClassInfo a = addClass(code, "O5", ClassType.CLASS);
		ClassInfo b = addClass(code, "O6", ClassType.CLASS);
		addRel(code, a, b, RelationshipType.ASSOCIATION);

		IntermediateModel uml = umlModel();
		addClass(uml, "O5", ClassType.CLASS);
		addClass(uml, "O6", ClassType.CLASS);
		// no edge in UML
		...
		Difference d = diffs.get(0);
		assertEquals(IssueKind.RELATIONSHIP_MISSING_IN_UML, d.getKind());
		assertEquals(IssueLevel.WARNING, d.getLevel()); // relaxed on relationships
	}
