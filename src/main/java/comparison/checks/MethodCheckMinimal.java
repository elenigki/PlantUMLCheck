...
public final class MethodCheckMinimal {
	...
	public static List<Difference> compareMethodsInClass(String className, ClassInfo codeC, ClassInfo umlC,
		...
		for (Method U : umlMethods) {
			...
			if (paramsOmitted) {
				...
				if (C == null) {
					out.add(new Difference(IssueKind.METHOD_MISSING_IN_CODE, IssueLevel.ERROR, whereNameOnly,
							"Method name present in UML but not found in code", "present", "missing",
							"Remove from UML or add method in code"));
					continue;
				}

				// Params omitted -> SUGGESTION
				out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, whereNameOnly,
						"Parameters omitted in UML", "omitted", SignatureRules.signatureOf(C),
				...
				if (uret.isEmpty()) {
					if (!cret.equals("void") && !cret.isEmpty()) {
						out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, whereNameOnly,
								"Return type omitted in UML", "omitted", cret, "Consider documenting the return type"));
					}
				} else if (!TypeRules.equalStrict(uret, cret)) {
					// Ambiguous due to omitted params -> keep it a SUGGESTION, not an ERROR
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, whereNameOnly,
							"UML return type may not match the code overload", uret, cret,
				...
				if (omitted(U.getVisibility())) {
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, whereNameOnly,
							"Method visibility omitted in UML", "omitted", cVis, "Optionally show the visibility"));
				} else {
					String uVis = VisibilityRules.vis(U.getVisibility());
					int ur = rank(uVis), cr = rank(cVis);
					if (ur > cr) {
						out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.ERROR, whereNameOnly,
								"UML visibility is more restrictive than code", uVis, cVis,
								"Make UML at least as visible as the code"));
					} else if (ur < cr) {
						out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.WARNING, whereNameOnly,
								"UML visibility is less restrictive than code", uVis, cVis,
								"Consider aligning UML visibility with code"));
					}
				}

				// Static (name-only): if omitted and code static => SUGGESTION; if written and
				// differs => ERROR
				...
				if (uStat == null) {
					if (codeStatic) {
						out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, whereNameOnly,
								"Static modifier omitted in UML for a static method", "static: omitted", "static: true",
								"Mark the UML method as static"));
					}
				} else if (uStat.booleanValue() != codeStatic) {
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.ERROR, whereNameOnly,
							"Static modifier mismatch between UML and code", "static: " + uStat,
							"static: " + codeStatic, "Make UML static match the code"));
				}
			...
			if (C == null) {
				out.add(new Difference(IssueKind.METHOD_MISSING_IN_CODE, IssueLevel.ERROR, where,
						"Method (signature) present in UML but missing in code", "present", "missing",
			...
			if (!uret.isEmpty()) {
				if (!TypeRules.equalStrict(uret, cret)) {
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.ERROR, where,
							"Return type differs (MINIMAL requires exact match when written)", uret, cret,
							"Align UML return type to code"));
				}
			} else {
				if (!cret.equals("void") && !cret.isEmpty()) {
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION,
							className + "#" + name + "(…)", "Return type omitted in UML", "omitted", cret,
			...
			String cVis = VisibilityRules.vis(C.getVisibility());
			if (omitted(U.getVisibility())) {
				out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.SUGGESTION, where,
						"Method visibility omitted in UML", "omitted", cVis, "Optionally show the visibility"));
			} else {
				String uVis = VisibilityRules.vis(U.getVisibility());
				int ur = rank(uVis), cr = rank(cVis);
				if (ur > cr) {
					...
					out.add(new Difference(IssueKind.METHOD_MISMATCH, IssueLevel.WARNING, where,
							"UML visibility is less restrictive than code", uVis, cVis,
							"Consider aligning UML visibility with code"));
				}
			}
		...
		for (Method C : codeMethods) {
			...
			if (matchedExact.contains(sigC))
				continue;
			if (umlNameOnly.contains(name))
				continue;

			String vis = VisibilityRules.vis(C.getVisibility());
			IssueLevel lvl = (vis.equals("+") || vis.equals("#")) ? IssueLevel.SUGGESTION : IssueLevel.INFO;

			out.add(new Difference(IssueKind.METHOD_MISSING_IN_UML, lvl, className + "#" + sigC,
					"Method missing in UML", "missing", "present",
	...
	static Method firstByName(List<Method> list, String name) {
		...
		for (Method me : list) {
			if (me == null)
				continue;
			if (safe(me.getName()).equals(name))
				return me;
		}
		return null;
	}
	...
	private static boolean omitted(String v) {
